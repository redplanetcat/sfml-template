...
PLATFORM_DRAW_VERTICES(PlatformDrawVertices) {
  ...
  PlatformWindow.draw(reinterpret_cast<sf::Vertex*>(Vertices), NumVertices, PlatformPrimitiveType, PlatformRenderStates);
}
...
PLATFORM_USE_SHADER(PlatformUseShader) {
  if (ShaderHandle == 0) {
    PlatformRenderStates.shader = nullptr;
    return;
  }
  if (PlatformShadersCount <= ShaderHandle) {
    return;
  }
  PlatformRenderStates.shader = &PlatformShaders[ShaderHandle].Shader;
}
...
PLATFORM_SET_SHADER_UNIFORM_VEC2(PlatformSetShaderUniformVec2) {
  if ((ShaderHandle == 0) || (PlatformShadersCount <= ShaderHandle)) {
    return;
  }
  sf::Shader& Shader = PlatformShaders[ShaderHandle].Shader;
...
PLATFORM_CREATE_TEXT(PlatformCreateText) {
  if (PlatformTextCount >= MAX_TEXTS) {
    return 0;
  }
  sf::Text& Text = PlatformText[PlatformTextCount];
  Text.setFont(PlatformDefaultFont);
  Text.setString(String);
  Text.setCharacterSize(Size);
  Text.setFillColor(sf::Color(Color.r, Color.g, Color.b, Color.a));
  return PlatformTextCount++;
}
...
PLATFORM_UPDATE_TEXT(PlatformUpdateText) {
  if ((TextHandle == 0) || (PlatformTextCount <= TextHandle)) {
    return;
  }
  sf::Text& Text = PlatformText[TextHandle];
  Text.setString(String);
  Text.setFillColor(sf::Color(Color.r, Color.g, Color.b, Color.a));
